class Car extends Entity {
	constructor(n_p, cx, cy, width, height, img, index, frontWeapon) {
		super(n_p, cx, cy, width, height, img);
		this.index = index;
	...
	mov_control() {
		if(keys[carsKeys[this.index].up]) {
			...
			this.velocidade -= this.aceleracao / 1.5 + this.velocidade / 100;
			this.remanc = 1;
		}
	...
	mov_aplic() {
		...
		this.direcao_alvo = this.angle * (Math.PI / 180) - Math.PI;
		...
		if(
			...
			if(ang_mov_1 > ang_mov_2) {
				let ind = (0.01 * (65 - ang_mov_2) * Math.abs(this.velocidade)) / 1.5;
				// Define derrapagem alvo
				this.anguloDerrapagemAlvo = -this.derra_mudanca_dir * ind * this.velocidade;
			} else {
				let ind = (0.01 * (65 - ang_mov_1) * Math.abs(this.velocidade)) / 1.5;
